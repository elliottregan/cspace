...
/**
 * Probe a supervisor's Unix socket for status. Returns { alive, ...fields }
 * or { alive: false, reason }. Mirrors cli.mjs trySocketRequest but inlined
 * to avoid a shared-module dependency.
 */
function trySocketStatus(sockPath, timeoutMs = 2000) {
  return new Promise((resolve) => {
    if (!fs.existsSync(sockPath)) {
      resolve({ alive: false, reason: 'socket not present' })
      return
    }
    ...
    const timer = setTimeout(
      () => finish({ alive: false, reason: 'timeout' }),
      timeoutMs,
    )
    let buf = ''
    conn.on('connect', () =>
      ...
      if (idx >= 0) {
        try {
          ...
          if (reply.ok && reply.status) {
            finish({ alive: true, ...reply.status })
          } else {
            finish({ alive: false, reason: reply.error || 'bad status' })
          }
        } catch (e) {
          finish({ alive: false, reason: `parse error: ${e.message}` })
        }
      }
    })
    conn.on('error', (e) =>
      finish({ alive: false, reason: e.message }),
    )
    conn.on('end', () =>
      finish({ alive: false, reason: 'connection closed' }),
    )
  })
}

/** Find the newest session-*.ndjson file for an instance. */
function findLatestSessionFile(eventLogRoot, instance) {
  if (!eventLogRoot) return null
...
function readTailEnvelopes(filePath, maxLines) {
  try {
    ...
    tools.push(
      ...
      tool(
        ...
        async ({ instance: targetInstance }) => {
          ...
          const sockPath = path.join(msgDir, targetInstance, 'supervisor.sock')
          const socketResult = await trySocketStatus(sockPath)
