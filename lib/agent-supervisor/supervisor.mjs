 ...
 *
 * Launched by `cspace up`, `cspace coordinate`, `cspace issue`, and the
 * teleport resume path. The supervisor:
 ...
 *      coordinators) where host-side `cspace send/respond/interrupt`
 *      commands are delivered as a new user turn (via queue.push) or an
 *      SDK interrupt
 *
 * Roles:
...
async function main() {
  ...
  if (inboxDir) {
    ...
    const scan = () => {
      ...
      for (const f of entries) {
        ...
        // Skip files older than --ignore-inbox-before to prevent directive
        // replay after a supervisor restart (the old session's directives
        // are stale but still sit in the inbox directory).
        if (args.ignoreInboxBefore > 0) {
          try {
            const mtime = fs.statSync(path.join(inboxDir, f)).mtimeMs
            if (mtime < args.ignoreInboxBefore) continue
          } catch {
            continue
          }
        }
  ...
  try {
    for await (const msg of queryHandle) {
      ...
      if (msg && msg.type === 'assistant') {
        turnCount += 1
        // Capture the latest assistant text for the completion summary so
        // the coordinator sees the agent's final diagnostic without having
        // to read the full event log.
      ...
      if (msg && msg.type === 'result') {
        ...
        // For now we end after the first result (one-shot semantics,
        // matching the way `cspace up` / `cspace issue` launch this).
        // Later we can add a --persist flag to keep the queue open for
        // mid-session injection without requiring a prompt-file.
        promptQueue.close()
      }
    }
  } catch (e) {
  ...
  // Release the socket and event stream BEFORE writing the completion
  // notification. This ensures that "completion file exists" reliably
  // implies "old supervisor has released its socket" — critical for the
  // restart_agent flow which polls for the completion file and then
  // immediately launches a new supervisor on the same socket path.
  // The process.on('exit', cleanup) handler (line 479) still fires but
  // is a no-op due to the cleanupDone flag.
  cleanup()
  ...
  if (args.role === 'agent' && args.instance) {
    ...
    const completion = {
      ...
      summary: lastAssistantText ? lastAssistantText.slice(0, 500) : null,
    }
