...
// Ignore EPIPE on stdout — the downstream pipe (stream-status.sh) may close
// before the supervisor finishes writing. Without this, an unhandled 'error'
...
function parseArgs(argv) {
  const args = { role: 'agent', idleTimeoutMs: 10 * 60 * 1000, eventLogDir: '/logs/events', ignoreInboxBefore: 0 }
...
Options:
  ...
  --ignore-inbox-before <ms>    Epoch-ms timestamp; inbox watcher skips files
                                with mtime before this. Used by restart-supervisor
                                to prevent directive replay from the prior session.`)
      process.exit(0)
    } else {
