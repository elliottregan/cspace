...
async function main() {
  ...
  //
  // For coordinators: the idle period happens *between* turns while waiting
  // for worker completions (via `cspace send _coordinator`). Interrupting a
  // waiting-for-input query is undefined, so we close the promptQueue instead
  // — the async iterator ends, the for-await loop exits, and the coordinator
  // shuts down cleanly. Each incoming worker message resets lastActivity via
  // the new query cycle it triggers.
  if (args.idleTimeoutMs > 0) {
    ...
    idleTimer = setInterval(() => {
      ...
      if (args.role === 'coordinator') {
        console.error(
          `[supervisor] coordinator idle for ${Math.round(idle / 1000)}s — no worker messages received, shutting down`,
        )
        promptQueue.close()
      } else {
        console.error(
  ...
  try {
    for await (const msg of queryHandle) {
      ...
      if (msg && msg.type === 'result') {
        ...
        if (args.role === 'coordinator') {
          // Coordinator stays alive between turns so worker completion
          // messages (arriving via `cspace send _coordinator` or the
          // inbox watcher) can wake it for the next query cycle. Reset
          // lastActivity so the idle timer counts from now — each
          // incoming worker message triggers a new turn which resets
          // it again. The session exits when the idle timer fires
          // (no messages for idleTimeoutMs) or on SIGTERM/SIGINT.
          lastActivity = Date.now()
          console.error(
            `[supervisor] coordinator turn complete (${turnCount} turns) — waiting for worker messages`,
          )
        } else {
