 ...
 * ProcessStream() expects from `claude --output-format stream-json`.
 *
 ...
 * `total_cost_usd`, while ProcessStream() reads `.cost_usd`. Everything
 * else (system/init, assistant, user, tool_use) passes through unchanged;
 * ProcessStream() tolerates extra fields and ignores unknown message
 * types (rate_limit_event, hook_started, hook_response, …).
 */

/**
 * Normalize an SDKMessage into the object shape ProcessStream() expects.
