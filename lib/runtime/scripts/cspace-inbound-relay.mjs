...
function startRelay(port, bindAddr) {
  ...
  server.on('error', (err) => {
    // Most likely EADDRINUSE: something claimed this address:port between the
    // poll and the bind. Drop it; the next reconcile re-evaluates.
    console.error(`relay ${port}: ${err.code || err.message}`);
    relays.delete(port);
    try { server.close(); } catch { /* already closing */ }
  });
  server.listen(port, bindAddr, () => console.log(`relay: ${bindAddr}:${port} -> 127.0.0.1:${port}`));
...
function reconcile() {
  ...
  for (const port of need) {
    if (!relays.has(port)) startRelay(port, bindAddr);
  }
