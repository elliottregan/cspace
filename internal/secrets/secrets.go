...
// ParseFile reads one dotenv-style secrets file. A missing file yields
// (nil, nil) — absence is not an error. Exported so internal/credentials
// can rank the legacy secrets.env layers without reimplementing the parser.
