...
make test-race    # -race over internal/pane + internal/controlplane + internal/control; NOT part of make check
CSPACE_CLIPBOARD_TESTS=1 go test ./internal/cli/ -run Clipboard   # pasteboard tests: overwrite the real clipboard (text is restored, an image is not); SKIP under a bare make check
