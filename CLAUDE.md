...
- **control** — the control API the TUI and CLI commands share: attach argv construction and TERM/COLORTERM mapping (`argv.go`), the tmux driver — presence probe, client list, detach (`tmux.go`) — and the attach lock plus per-client records under `~/.cspace/controlplane/<project>/<sandbox>/` (`attach.go`). `cmd_attach.go` and the TUI's `Attach` action both call into it rather than duplicating the argv/lock/detach logic. It has since grown to hold every query (`Snapshot`, `AgentStatus`, `Events`, `InteractiveState`, `Ports`) and every action (`Down`, `Send`, `Interrupt`, `RestartBrowser`, `Up`, `ListClients`, `DetachClient`) behind one `Client`/`Options`, seamed on `ContainerCLI` (the substrate), `EntryStore` (the registry) and `Host` (the operations — teardown, browser restart — whose implementations still live in `internal/cli`, since `Up` shells out to the running binary rather than call a cobra command directly). `internal/tui` no longer has its own copies of these; it consumes the package through type aliases (`internal/tui/types.go`), so a `Row` built there is the same type as one built here.
