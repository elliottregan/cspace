...
- `daemonDNSHandler` (internal/cli/cmd_daemon.go): in the 2-label case (`<sandbox>.<project>`), when the leftmost label is the reserved word `browser`, resolve the container `cspace-<project>-browser` via the existing `lookupSidecarIP` path (TTL memo + negative cache apply) and skip the registry-entry match. No registry entry, no lifecycle bookkeeping — if the container exists, the name resolves; after a restart the new IP propagates within one memo TTL.
